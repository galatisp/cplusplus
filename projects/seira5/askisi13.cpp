...
void decode(char *input, char *output, char *code)
{
    ...
    do
    {
        ...
        if (input_c != 0)
        {
            ...
            if (input_c >= 'a' && input_c <= 'z')
            {
                pos = getpos(code, input_c);
                output[i] = 'a' + pos;
            }
            else if (input_c >= 'A' && input_c <= 'Z')
            { //capitals
                pos = getpos(code, input_c + 32);
                output[i] = 'A' + pos;
            }
            else
                output[i] = input[i];
...
int main()
{
    ...
    get_from_user(action);

    get_from_user(input);
    if (action[0] == 'd')
    { 
          cout << "Decoding: " << endl;
        decode(input, output, code);
    }
